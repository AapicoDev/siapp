...
export default function Sidebar() {
  ...
  const menuList = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          submenu: true,
          submenuItems: [
            {title: 'Segment', path: "/masterData/segment", icon: <GoDotFill/>},
            ...
            {title: 'Manage Sync Employee Department Mapping', path: "#", icon: <GoDotFill size={26}/>},
          ]
        },
        {
          path: '#',
    ...
    return (
      ...
      <CommandList style={{overflow: 'visible'}}>
          ...
          <CommandItem key={optionkey} 
                       className={`${pathName.includes(option.pathname) ? "font-bold menuSelected" : ""} py-2 flex gap-2 pl-2`}
                       >
            {pathName.includes(option.pathname) ? option.iconSelected : option.icon}
            {option?.text}
            <FaSortDown size={20} 
             className={`${subSIAPPOpen&&(option.text==='SIAPP') || 
                          subReportOpen&&(option.text==='Report' ) || 
                          subMaterDataOpen&&(option.text==='Master Data' ) || 
                          subUserOpen&&(option.text==='User' ) ? 
                          "rotate-180 mt-2" : "mb-2"} ml-auto stroke-2 text-xs`}/>
          </CommandItem>
          </div>
          
          {(option.text === 'SIAPP') && 
          <Collapse in={subSIAPPOpen} timeout={'auto'} unmountOnExit>
            <CommandList className="border-b mb-1" style={{overflow: 'visible'}}>
              {option.submenuItems?.map((submenu: any, submenukey: number) => (
                <Link key={submenukey} href={submenu.path}>
                  <CommandItem key={submenukey} className={`${pathName === submenu?.path ? "bg-accent border-r-8 font-bold" : ""} gap-2 px-7 py-3 leading-5 flex1`}>
                    {submenu?.icon}
                    {submenu?.title}
                  </CommandItem>
                </Link>
          ...
          <Collapse in={subReportOpen} timeout={'auto'} unmountOnExit>
             <CommandList style={{overflow: 'visible'}} className="border-b mb-1">
              {option.submenuItems?.map((submenu: any, submenukey: number) => (
                <Link key={submenukey} href={submenu.path}>
                  <CommandItem key={submenukey} className={`${pathName === submenu?.path ? "bg-accent border-r-8 font-bold" : ""} gap-2 px-7 py-3 leading-5 flex1`}>
                    {submenu?.icon}
      ...
      (<Link key={optionkey} href={option?.path}>
          <CommandItem key={optionkey} className={`${pathName === option?.path ? "bg-accent border-r-8 font-bold" : ""} py-2 flex gap-2 pl-2`}>
  ...
  return (
    <div className="w-[260px] min-w-[260px] min-h-screen border-r">
