import { account, createDocumentOnServer, databases, fetchData, fetchDataList, fetchOneData } from "../appwrite";
...
export async function getPatrolCheckList(checkpointId) {
    ...
    try {
        const response = await fetchDataList(databaseId, patrolChecklistTableId, [Query.equal('CheckpointId', checkpointId)]);
        console.log(response);
        return response;
    } catch (error) {
        console.error('Error retrieving data:', error)
...
export async function getMasterRoundData(areaId) {
    try {
        const response = await fetchDataList(databaseId, masterRoundTableId, [Query.equal('areaId', areaId)]);
        console.log(response);
        return response;
    } catch (error) {
        console.error('Error retrieving data:', error)
